...
- [x] 1.1 Add `src/lib/faq.ts` with typed `faqContent`: documentTitle, description, heading, intro, ≥5 dummy Q&A items covering pricing, timing, ownership, revisions, support, and starting a project, callout, cta label “Start a Project” and href `/contact`. Dummy copy in the homepage studio voice; no lorem, numeric prices, SLAs, calendar guarantees, or named people
- [x] 1.2 Point nav “FAQ” at `/faq`. Leave Process on `/process`, Studio on `/studio`, footer Technology on `/technology`

## 2. FAQ page

- [x] 2.1 Add `/faq` that reads only `faqContent` for metadata and body copy (no extra FAQ strings in the page file)
- [x] 2.2 Render heading, intro, Q&A cards (mist 24px), peach callout, “Start a Project” pill from the document. Stay on Steep; at most one peach card
